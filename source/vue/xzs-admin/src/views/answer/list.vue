<template>
  <div class="app-container">
    ...
    <el-table v-loading="listLoading" :data="tableData" border fit highlight-current-row style="width: 100%">
      ...
      <el-table-column  label="系统评分" width="100px" >
        <template slot-scope="{row}">
          {{row.systemScore}} / {{row.paperScore}}
        </template>
      </el-table-column>
      <el-table-column  label="教师评分" width="100px" >
        <template slot-scope="{row}">
      ...
      <el-table-column  align="right" label="批改状态" width="70">
        <template slot-scope="{row}">
          <router-link target="_blank" :to="{path:'/editPaper',query:{id:row.id}}" v-if="row.status === 1 ">
            <el-button  type="text" size="small">批改</el-button>
          </router-link>
          <router-link target="_blank" :to="{path:'/readPaper',query:{id:row.id}}" v-if="row.status === 2 ">
